...
- `counts_by_window(today): dict` — returns per-window counts and all-sorted flags
...
- Filter pane — `PaneHeader` "Inbox" with scan controls (spinner + elapsed + cancel while scanning; "Scan" button + "More" dropdown with "Clear scan results" when idle); `ListView` of `InboxTypeRow` items (Today, Yesterday, Last 7 days, Last 30 days) with count and all-sorted indicator
