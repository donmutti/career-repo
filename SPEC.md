...
Flow — horizontal or vertical step-flow indicator. Steps render inside a single `border rounded-full overflow-hidden` container so the outer shape is a pill; each step is a rectangular clickable tab (steps share the container's clip — the first/last edges become half-pills naturally; no per-step rounding). Active step highlighted with `bg-action/10` + `text-action`. No separator between steps. Each step may render its `icon` when provided.
...
- User clicks the `Star` `IconButton` in the `JobView` toolbar, or the leading star in a `JobRow`. Toggles `is_starred` via `PATCH /opportunities/{id}` with `{is_starred: <bool>}`. Starred items surface when the `Starred` time-window option is selected, regardless of status or archived state.
