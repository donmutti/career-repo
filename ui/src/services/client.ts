...
export const opportunities = {
  ...
  generateCoverLetter: (id: string, body?: {instructions?: string}) => apiFetch<{run_id: string}>(`/opportunities/${id}/cover-letter`, {method: 'POST', body: body ? JSON.stringify(body) : undefined}),
...
export const inbox = {
  ...
  scan: (params?: { last_scanned_at?: string }) => apiFetch<unknown>('/inbox/scan', {method: 'POST', body: JSON.stringify(params ?? {})}),
  ...
  patchEmailOpportunity: (eo_id: string, data: { status: string; opportunity_id?: string; reason?: string | null }) => apiFetch<unknown>(`/inbox/opportunities/${eo_id}`, {method: 'PATCH', body: JSON.stringify(data)}),
  declineReasons: () => apiFetch<unknown>('/inbox/decline-reasons'),
